...
@app.route('/comment/task/<int:task_id>', methods=['GET', 'POST'])
@login_required
def comment_on_task_page(task_id):
    task = Todo.query.get_or_404(task_id)
    if request.method == 'POST':
        content = request.form.get('content')
        if content:
            ...
            return redirect(url_for('dashboard'))
    return render_template('comment_on_task.html', task=task)
...
def dashboard():
    ...
    non_followed_users = User.query.filter(User.id != user_id)\
                                   .filter(~User.id.in_(db.session.query(Follow.followee_id)\
                                   .filter(Follow.follower_id == user_id)))\
                                   .all()

    user_achievements = Achievement.query.join(UserAchievement)\
    ...
    return render_template("dashboard.html", 
                           ...
                           all_tasks=all_tasks)  # 👈 Pass it to the template
